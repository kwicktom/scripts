...
def readFile(file, enc=False):
    buffer=[]
    f = openFile(os.path.join(file), 'r', enc)
    buffer.append(f.read())
    f.close()
    return buffer

def writeFile(file, text, enc=False):
    f = openFile(os.path.join(file), 'w', enc)
    f.write(text)
    f.close()
